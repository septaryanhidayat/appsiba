...
class VisitorAnalyticsTest extends TestCase
{
    ...
    public function test_visitor_tracking_debounces_rapid_reloads_on_same_page(): void
    {
        ...
        // Immediate 2nd request with same session / ip
        $this->withHeaders($headers)->get('/berita');
    ...
    public function test_unauthenticated_user_cannot_access_analytics_dashboard(): void
    {
        ...
        $responseAlias = $this->get('/admin/analitik-pengunjung');
        $responseAlias->assertRedirect('/login');
    }
    ...
    public function test_authenticated_admin_can_access_analytics_dashboard(): void
    {
        ...
        VisitorLog::create([
            ...
            'browser' => 'Google Chrome',
            'platform' => 'Windows',
            'referrer' => 'https://google.com/search?q=appsi',
        ...
        $response = $this->actingAs($admin)->get('/admin/analitik');

        $response->assertStatus(200);
        $response->assertSee('Statistik Pengunjung', false);
        $response->assertSee('Analitik Portal', false);
        $response->assertSee('Tren Kunjungan', false);
        $response->assertSee('Perangkat', false);
        $response->assertSee('Browser Pengunjung', false);
        $response->assertSee('Asal Lalu Lintas Pengunjung', false);
        $response->assertSee('Feed Kunjungan Terkini', false);
    ...
    public function test_visitor_tracker_service_user_agent_and_referrer_parsing(): void
    {
        ...
        $this->assertEquals('Mesin Pencari (Search)', VisitorTrackerService::classifyReferrerType('google.com', 'https://www.google.com/search?q=appsi+banyuasin'));
        $this->assertEquals('Media Sosial', VisitorTrackerService::classifyReferrerType('l.facebook.com', 'https://l.facebook.com/l.php?u=https://appsiba.or.id'));
        ...
        $this->assertEquals('Situs Eksternal', VisitorTrackerService::classifyReferrerType('banyuasinkab.go.id', 'https://banyuasinkab.go.id/berita-pasar'));

        // 4. Real visitor stats calculation returns expected structure
